...
WebView上でキーボードショートカットを使ってMarkdownファイルを保存できるようにする。

#### 仕様

- **ショートカット**: `Ctrl+S`（macOSでは`Cmd+S`）
- **動作**: 既存の保存処理（`saveDocument()`）を呼び出し
- **Dirty状態の場合**: ファイルを保存し、FloatingActionsが非表示になる
- **Dirty状態でない場合**: 何もしない（既に保存済み）

#### 実装方針

WebView側でキーボードイベントをキャプチャし、既存の保存アクションを呼び出す。
...
- **FloatingActions**: 引き続き表示される（isDirty=true時）。ショートカットは追加の保存手段
- **Dirty表示**: 現状のFloatingActionsの表示/非表示で十分（追加のUI変更なし）
- **保存通知**: FloatingActionsが消えることで保存完了がわかる（追加の通知なし）
